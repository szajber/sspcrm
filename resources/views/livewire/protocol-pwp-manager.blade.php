<div>
    <div class="flex justify-between items-center mb-4">
        <div>
            <h4 class="text-md font-semibold text-gray-700">{{ __('Lista Urządzeń PWP') }}</h4>
                    ...
                    @foreach($devices as $index => $device)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                PWP {{ $device->system_number }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ $device->type === 'switch' ? __('Wyłącznik') : __('Punkt aktywacji') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $device->location }}
                            </td>
    ...
    <x-dialog-modal wire:model="showModal">
        <x-slot name="title">
            {{ $editingId ? __('Edytuj Urządzenie PWP') : __('Dodaj Urządzenie PWP') }}
        </x-slot>
        ...
        <x-slot name="content">
            <div class="grid grid-cols-1 gap-6">
                <!-- System Number -->
                <div>
                    <x-label for="system_number" value="{{ __('Numer Systemu PWP') }}" />
                    ...
                    <p class="text-xs text-gray-500 mt-1">{{ __('Grupuj urządzenia w ramach jednego systemu PWP (np. 1, 2).') }}</p>
                </div>
                ...
                <div>
                    ...
                    <select wire:model="type" id="type" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                        <option value="switch">{{ __('Wyłącznik prądu') }}</option>
                        <option value="trigger">{{ __('Punkt aktywacji (Wyzwalacz)') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <x-input wire:model="location" id="location" type="text" class="block mt-1 w-full" placeholder="np. Rozdzielnia główna, Garaż" />
